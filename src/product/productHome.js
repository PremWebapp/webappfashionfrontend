    ...
    return (
        <div className=" overflow-hidden ">
            ...
            {isLoading ? <div className='home-product-loader p-5'>
                ...
                <div className="row row-cols-sm-2 row-cols-md-3 row-cols-lg-5 row-cols-xl-4   g-2 m-1">
                    {filteredItems.map(item => {
                        return <div className='col d-flex justify-content-center cursor-pointer-card' onClick={()=>dispatch(selectedProduct(item))}> <ProductCard rating={item.rating.rate} count={item.rating.count} price={item.price} image={item.image} description={item.description}>{item.title}</ProductCard></div>
                    })}
                </div>}
        </div>
    )
